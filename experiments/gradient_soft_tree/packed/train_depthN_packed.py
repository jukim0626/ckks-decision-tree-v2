...
사용법: python -m experiments.gradient_soft_tree.packed.train_depthN_packed wine 3 10 2.0 0 17
...
def train(
    ...
    _run(
        "experiments.gradient_soft_tree.setup_worker_N",  # baseline 그대로 재사용
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for attempt in range(1, _MAX_OOM_RETRIES + 2):
            _wait_for_gpu_settle()
            try:
                _run("experiments.gradient_soft_tree.packed.epoch_worker_packed", str(session_dir))
                break
    ...
    _wait_for_gpu_settle()
    stdout = _run("experiments.gradient_soft_tree.finalize_worker_N", str(session_dir), str(n_epochs))  # baseline 그대로 재사용
