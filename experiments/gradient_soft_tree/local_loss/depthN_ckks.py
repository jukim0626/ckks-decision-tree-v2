...
from experiments.gradient_soft_tree.local_loss.depthN_reference import train_depthN_local_loss  # noqa: E402
...
def forward_backward_update_N(ctx, dataset, params: dict, sample_mask, n_features: int, n_classes: int, depth: int, lr: float):
    ...
    for level in range(depth):
        ...
        # ---- forward: baseline depthN_ckks.py의 이 레벨 부분과 100% 동일한 gate 계산 ----
        ...
        for c in range(n_classes):
            ...
            for k in range(n_virtual_leaves):
                ld_c = _ensure_level(ctx, _extract_weight_broadcast(ctx, local_dist[k], c))
        ...
        for k in range(n_virtual_leaves):
            ...
            for c in range(n_classes):
                ...
                ld_c = _extract_weight_broadcast(ctx, local_dist[k], c)
