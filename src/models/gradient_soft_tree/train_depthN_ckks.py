...
사용법: python -m models.gradient_soft_tree.train_depthN_ckks iris 3 35 2.0 0
...
def train(dataset_name: str, depth: int, n_epochs: int, lr: float, seed: int, level_preset: int | None = 17) -> dict:
    ...
    for epoch in range(1, n_epochs + 1):
        _wait_for_gpu_settle()
        t0 = time.time()
        _run("models.gradient_soft_tree.epoch_worker_N", str(session_dir))
        elapsed = time.time() - t0
        print(f"[{dataset_name} depth={depth}] epoch {epoch}/{n_epochs} done | {elapsed:.1f}s", flush=True)

    _wait_for_gpu_settle()
    stdout = _run("models.gradient_soft_tree.finalize_worker_N", str(session_dir), str(n_epochs))
