"""batched_argmin_v3 (cross-batch 먼저, block reduction 나중) vs 기존 파이프라인 비교."""
...
from batched_argmin_v3 import train_fully_encrypted_mgi_stump_batched_v3
...
t0 = time.time()
_, winner_aux2 = train_fully_encrypted_mgi_stump_batched_v3(ctx2, dataset2, candidates, normalizer, SHARPEN, verbose=True)
time2 = time.time() - t0
...
print(f"\n[batched v3 파이프라인] time={time2:.2f}s | picked_feature={picked2} (conf={feat_conf2[picked2]:.3f})")

print(f"\n=== 결과 ===")
print(f"plaintext best feature={candidates[best_idx].feature_idx}")
print(f"기존: {time1:.2f}s | batched v3: {time2:.2f}s | speedup={time1/time2:.2f}x")
